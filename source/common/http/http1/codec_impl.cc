...
int ServerConnectionImpl::onHeadersComplete() {
  ...
  // sense.
  if (active_request_) {
    auto& headers = absl::get<RequestHeaderMapImplPtr>(active_request_->headers_or_trailers_);
  ...
  if (active_request_) {
    ...
    if (deferred_end_stream_headers_) {
      active_request_->request_decoder_->decodeHeaders(
          std::move(absl::get<RequestHeaderMapImplPtr>(active_request_->headers_or_trailers_)),
          true);
    } else if (processing_trailers_) {
      active_request_->request_decoder_->decodeTrailers(
          std::move(absl::get<RequestTrailerMapImplPtr>(active_request_->headers_or_trailers_)));
    } else {
...
int ClientConnectionImpl::onHeadersComplete() {
  ...
  if (pending_responses_.empty() && !resetStreamCalled()) {
    ...
    auto& headers =
        absl::get<ResponseHeaderMapImplPtr>(pending_responses_.front().headers_or_trailers_);
  ...
  if (!pending_responses_.empty()) {
    ...
    if (deferred_end_stream_headers_) {
      response.decoder_->decodeHeaders(
          absl::get<ResponseHeaderMapImplPtr>(std::move(response.headers_or_trailers_)), true);
    } else if (processing_trailers_) {
      response.decoder_->decodeTrailers(
          std::move(absl::get<ResponseTrailerMapImplPtr>(response.headers_or_trailers_)));
    } else {
      Buffer::OwnedImpl buffer;
